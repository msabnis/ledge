...
### Build order
- [x] Scaffold via Shopify CLI template, Prisma schema (Postgres), core lib (calc engine + invoice parser ported), webhook handlers, and all five Polaris screens — pushed to GitHub. `npm install`, `tsc --noEmit`, and `npm run build` all pass clean in the repo as of this commit.
- [ ] Run `npx prisma generate` and `npx prisma migrate dev` locally (needs real network access to Prisma's engine CDN — the sandbox this was built in couldn't reach it, so this hasn't been verified end-to-end yet)
- [ ] Create the app in the Shopify Partner Dashboard, run `shopify app config link` locally, confirm OAuth install works end-to-end on a Partner dev store
- [ ] Connect Railway: add Postgres plugin, set env vars (`SHOPIFY_API_KEY`, `SHOPIFY_API_SECRET`, `SCOPES`, `SHOPIFY_APP_URL`, `DATABASE_URL`), connect GitHub repo for auto-deploy on push to `master`
...
- [ ] Verify the backfill GraphQL query against the live Admin API schema (field names can drift between API versions — check in the GraphiQL explorer before running against a real merchant's store)
